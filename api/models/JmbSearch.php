...
class JmbSearch extends Jmb
{
    ...
    {
        $query = Jmb::find()->where(['status' => 1])->orderBy('id desc');

        $search = $params['JmbSearch']['search'] ?? '';
        ...
        if($search){
            $data = JmbCategoryModel::find()->where(['like', 'name', $search])->select('id')->andWhere(['<>', 'pid', 0])->asArray()->all();
            $ids = array_column($data, 'id');

            $query->andFilterWhere(['or',
                ['like', 'name', $search],
                ['in', 'jmb_category_id', $ids]
            ]);
        }
